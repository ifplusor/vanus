...
func (rn *RawNode) Propose(opts ProposeOptions) {
	rn.raft.Propose(opts)
}
...
func (rn *RawNode) ReportCheckpointStatus(index uint64) error {
	return rn.raft.Step(pb.Message{
		Type:  pb.MsgCheckpointStatus,
		Index: index,
	})
}

func (rn *RawNode) UnsealedEntries() (uint64, []pb.Entry, error) {
	return rn.raft.raftLog.unsealedEntries()
}
