...
func TestProgressLeader(t *testing.T) {
	...
	require.Zero(t, r.prs.Progress[1].Match)

	ents := r.raftLog.unstable.entries
	require.Len(t, ents, 6)
	require.Len(t, ents[0].Data, 0)
	require.Equal(t, "foo", string(ents[5].Data))

	// The entries have been persisted.
	k.advance(r, s)

	require.Equal(t, uint64(6), r.prs.Progress[1].Match)
	require.Equal(t, uint64(7), r.prs.Progress[1].Next)
}

// TestProgressResumeByHeartbeatResp ensures raft.heartbeat reset progress.paused by heartbeat response.
func TestProgressResumeByHeartbeatResp(t *testing.T) {
	r := newTestRaft(1, 5, 1, newTestMemoryStorage(withPeers(1, 2)), newTestKeeper())
	r.becomeCandidate()
	r.becomeLeader()
	...
	assert.True(t, r.prs.Progress[2].ProbeSent)

	r.prs.Progress[2].BecomeReplicate()
	// assert.False(t, r.prs.Progress[2].ProbeSent)
	// r.prs.Progress[2].ProbeSent = true
...
func newTestConfig(id uint64, election, heartbeat int, storage Storage, keeper Keeper) *Config {
	return &Config{
		ID:              id,
...
func newTestRaft(id uint64, election, heartbeat int, storage Storage, keeper Keeper) *raft {
	return newRaft(newTestConfig(id, election, heartbeat, storage, keeper))
}
